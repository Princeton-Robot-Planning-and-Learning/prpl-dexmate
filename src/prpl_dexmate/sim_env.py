"""Gymnasium env wrapping a kinder sim as the world.

The fake/real path goes ``Interface -> RealVegaEnv (VegaObservation) ->
VegaMotion3DPerceiver (ObjectCentricState) -> Agent``. In sim mode the
kinder env already produces an ObjectCentricState, so re-encoding it as a
VegaObservation just to decode it again would be lossy nonsense. This
wrapper exposes the kinder env's state directly to the Runner; pair it
with the pass-through perceiver and plan executor under
``prpl_dexmate.real_sim`` to drive the agent loop without leaving sim.
"""
...
class KinderSimEnv(
    ...
    """Thin wrapper that devectorizes obs from a kinder env.

    Pass the gymnasium id (e.g. ``"kinder/VegaMotion3D-v0"``). The wrapped
    env owns the kinder state; this class just forwards ``reset`` /
    ``step`` / ``render`` and exchanges the raw vectorized obs for an
    ``ObjectCentricState``.
    """
    ...
    def __init__(self, env_id: str, **make_kwargs: Any) -> None:
        ...
        # Expose the inner env's spaces so callers that introspect the
        # spaces see something meaningful. The observation_space is the
        # *vectorized* box space even though step returns a devectorized
        # ObjectCentricState.
